...
# 미니맥스 알고리즘을 사용하는 AI 클래스
class MinimaxAI:
    def __init__(self, board_size, max_depth):
        self.board_size = board_size
        self.max_depth = max_depth

    # 미니맥스 알고리즘을 사용하여 최선의 수를 찾음
...
def play_games(num_games):
    ...
    for _ in range(num_games):
        ...
        for _ in range(100):
            ...
            minimax_ai = MinimaxAI(board_size, max_depth)
            monte_carlo_ai = MonteCarloAI(board_size, simulations)

            while state.check_win() == 0:
                if state.current_player == 1:
                    move = minimax_ai.get_best_move(state)
                else:
        ...
        minimax_total_wins += minimax_wins
        monte_carlo_total_wins += monte_carlo_wins
        draws += draw_count

        print(f"Game: {_ + 1}")
        print("Minimax AI Wins:", minimax_wins)
        print("Monte Carlo AI Wins:", monte_carlo_wins)
        print("Draws:", draw_count)
        print()

    print("=== Final Results ===")
    print("Minimax AI Wins:", minimax_total_wins)
    print("Monte Carlo AI Wins:", monte_carlo_total_wins)
    print("Draws:", draws)

    print("=== Final Results ===")
    print("Minimax AI Win Rate:", minimax_total_wins / (num_games * 100) * 100, "%")
